...
estimates of the time-since-collision are $0.46\pm^{0.09}_{0.16}$ Gyr and
...
shock velocity is approximately equal to or smaller than the collisional velocity of the
...
was discovered via its Sunyaev-Zel'dovich effect by the Atacama Cosmology Telescope (ACT;
...
to be undergoing a major merger approximately on the plane of the sky (\citealt{M12}, hereafter M12). El Gordo possesses a range of noteworthy features that allow us to constrain the merger dynamics in multiple ways. Ever since its discovery, there has been an ongoing effort for
...
to have caused this morphology \citep{M12}. 
...
\begin{figure}
	...
	\caption{Configuration of El Gordo showing overlay of dark
		...
		\label{fig:config}}
\end{figure}
El Gordo is one of $\sim 50$ of galaxy clusters ($\sim50$) that have
...
values, such as time-since-collision to be larger than the age of universe at the
...
impact parameter. The simulations of \cite{Ricker98} of bimodal mergers of dark matter
component with $10^{15} M_{\sun}$ each, showed that the resulting relative
velocity would be approximately at $2000 \text{ km s}^{-1}$ even if they vary the
impact parameter between 0 to 5 times the scale radius ($5~r_s = 3.5$ Mpc). \cite{Mastropietro2008a} also reported that
...
the corresponding characterisic core
radius of the NFW halo with the mass of the SE subcluster. We consider
the result from \citealt{Molnar14} may be due to incomplete exploration of
the parameter space, and that other impact parameter values may also match the
...
We provide two versions of the time-since-collision variables $TSC_{\rm out}$ and
$TSC_{\rm ret}$ to denote different possible merger scenarios. 1) We call the scenario for which the subclusters are
moving apart after collision to be ``outgoing" and it corresponds to the
smaller $TSC_{\rm out}$ value, and 2) we call the alternative scenario 
``returning" for which the subclusters are approaching each other after turning
around from the apocenter for the first time and it corresponds to $TSC_{\rm ret}$.
...
30\% from private communication with S. Paul and $\sim 10\%$ from
...
$TSC_{\rm out}$ and $TSC_{\rm ret}$ values. We worked in the center of mass frame where the
...
\end{equation}
\par 
We examined the projected separation for $0.7 \leq \beta
\leq
...
propagate with $\beta > 1$. Examples of
merging clusters with $\beta \approx 1.5$ is reported by cosmological
simulations \citep{Paul2011b}. \par 
However, we note that merger shocks from cosmological
simulations are reported to show low Mach numbers between 1 and 3
...
phase with an estimate of $TSC_{\rm ret} = 0.91\pm^{0.22}_{0.39}$ Gyr. This puts the
...
the time-since-collision, with $TSC_{\rm out} = 0.46\pm^{0.09}_{0.16}~\giga \text{yr}$
and $TSC_{\rm ret} =0.91\pm^{0.39}_{0.22}~\giga\text{yr}$ for the returning model. Both the estimates of
$TSC_{\rm out}$ and $TSC_{\rm ret}$ are compatible with the physical time-scales of observable
...
($TSC_{\rm ret}$) vs. the 3D velocity at the time of collision for El Gordo. The
...
that \cite{Donnert13} assumed $\alpha = 0$. \par 
...
The $TSC_{\rm out}$ from \cite{Donnert13} is below the estimated 95\% CI of
$TSC_{\rm out}$ from our work.
...
outgoing scenario of $TSC_{\rm out} / T$ indicated by the grey dotted line). Ram
...
Cluster is seen at a phase of $TSC_{\rm out} / T \approx ~0.4\text{ Gyr} / ~1.6
...
indicated by $TSC_{\rm ret} / T$ rather than $TSC_{\rm out} / T$), explaining why the DM of
...
In particular, the inferred outbound $TSC_{\rm out} / T \sim 0.3$ of the Bullet Cluster and El Gordo
are similar. If instead, the El Gordo
is in the returning phase of the merger (i.e. $TSC_{\rm out}$ for El Gordo is
...
between the number density peak and the luminosity peak is due to a much
brighter brightest-cluster-galaxy (BCG) located close to the corresponding DM peak.
Before there is a
...
\begin{equation}
	w(TSC_{\rm out}) = 
	\begin{cases}
		& \text{const}~\text{if }TSC_{\rm out} < \text{age of universe at } z=0.87	\\
		& 0~\text{otherwise}.
	\end{cases}
\end{equation}
In addition, we apply the following weight on $TSC_{\rm ret}$ only when evaluating the
statistics of $TSC_{\rm ret}$, thus allowing realizations with a valid
outgoing TSC but an invalid returning $TSC_{\rm ret}$. 
\begin{equation}
	w(TSC_{\rm ret}) = 
	\begin{cases}
		& \text{const}~\text{if }TSC_{\rm ret} < \text{age of universe at } z=0.87	\\
...
\begin{equation}
	w(TSC_{\rm out} | T) = 2 \frac{TSC_{\rm out}}{T},
\end{equation}
...
timescales. Note that there is a default weight for constraining $TSC_{\rm out}$ but
not for $TSC_{\rm ret}$ and $T$, so $TSC_{\rm out}$ spans a smaller range.}
	\end{center}
\end{minipage}
\end{figure*}
\begin{figure*}
