...
in the center of mass (momentum) frame and calculate how far the shock would have propagated for our inferred
...
outgoing and returning scenario favors the outgoing scenario.
...
assume $\langle v_{relic} \rangle / v_{3D, NW}(t_{col}) = 1.5$ (uppermost
panel of Figure \ref{fig: positionprior}), $\langle v_{relic} \rangle$
corresponds to an time-averaged velocity of $\sim 3800~\kilo
...
^{400}_{300} \kilo \meter~\second^{-1}$ relative to the surrounding intracluster medium.
...
\item favors an outgoing scenario if the time-averaged merger shock
	velocity in the center of mass frame is $>> ~\kilo \meter~\second^{-1}$ between the collision and the
observation. 
\end{enumerate}
