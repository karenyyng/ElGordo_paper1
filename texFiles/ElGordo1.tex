...
realizations as applying (importance) weights. 
...
SE relic. This conclusion favoring the returning case ($M_{ret}$) holds true for the
...
SE relic, showing that our results favor the returning scenario despite the
uncertainties. (See appendix~\ref{app:Bayes_factor}). Additionally, we
...
polarization weight and the colored contours correspond to the credible
regions after applying the weights. The contours represents the 95\% and
...
comments from Maru\v{s}aBrada\v{c} about using the position of the relic to
...
functional form of the likelihood (i.e. no Gaussian, or binomial
likelihood) in our context. We make use of $f(S_{proj} \cap S_{obj})$ to
...
Wald statistic is usually used in two-tail hypothesis test
This allows us to ask the question, given a particular model (merger
scenario),
