...
Here, we compare the different merger scenarios through several
...
\begin{align} 
	&P(s_{proj} \text{ compatible with }s_{obs} | M)  \\
...
This differs from the traditional hypothesis testing or model comparison in several ways: 
\begin{enumerate}
\item we did not compute the traditional likelihood function.  We have adopted
a non-parametric approach in our Monte Carlo simulation, there is no
parametric form of the likelihood in our context. Readers can feel
free to assume $f(S_{proj} \cap S_{obj})$ as a form of likelihood or
distance measure to compare our simulated and our observed data, but
coming up with a rigorous measure is beyond the scope of this paper
\item we are not asking whether the expected value of the radio relic such
	as the mean or median from each model match the observation best, since
	those estimators take into account the values that do not match the
	observed location of the radio relic 
\item we have marginalized the uncertainty in $\beta$ to be as conservative
