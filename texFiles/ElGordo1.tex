...
Gordo]{The dynamics and merging scenario of the galaxy cluster ACT-CL J0102-4915, El Gordo}
...
corresponding subcluster $< 1100~ \kilo\meter~\second^{-1}$ in the center of mass
...
index gradients and are identified as radio relics associated with shockwaves
...
phase with a estimate of $TSC_1 = 0.91\pm^{0.22}_{0.39}$ Gyr. This puts the
...
galaxy number density peak and the SE DM centroid, while almost no offset
...
\begin{enumerate}
	\item demonstrated the first use of polarization fraction information from
		...
		showed that this prior helps reduce uncertainty for the dynamical
...
comparable with data to the questions above. Metastudies of merging
clusters in multiple wavelengths supported with simulations such as this
work would allow us to understand the merger clusters well enough to push the bounds on
$\sigma_{\text{SIDM}}$. \par 
...
\begin{figure}
	\includegraphics[width=\linewidth]{wald.eps}
	\caption{Wald statistic to show whether the observed relic location is
		within the confidence interval of each merger scenario model. (lower
		value
		means more compatible)  
	\label{fig:waldtest}}
\end{figure}
Here, we compare the different merger scenarios through two statistical 
quantities and show that they consistently give the conclusion that the returning
scenario is favored. The first quantity that we use answers the question:
which scenario would have a higher (marginalized) probability of producing simulated
values ($s_{proj}$)
compatible with the observed location of the radio relic ($s_{obs}$). 
...
\begin{align} 
	...
	d\beta, 
\end{align}
where $f$ indicates the corresponding probability density function, $M$
represents one of the merger scenarios, and $\beta$ is defined in equation
\ref{eqn:NW_speed} and \ref{eqn:SE_speed},
$s_{proj} \in S_{proj}$ and $s_{obj} \in S_{obs}$. 
And we found: 
\begin{align}
...
scenario. 
And we have set our priors set to be uniform: 
\begin{equation}
...
functional form of the likelihood (i.e. no Gaussian, or binomial likelihood) in our context. Readers can feel
free to assume $f(S_{proj} \cap S_{obj})$ to be a (zero-one) loss function
analog or a distance measure to compare our simulated and our observed data
...
\end{enumerate}
\par
The second quantity that we compute is the Wald statistic
(two-tail hypothesis test) at a range of possible $\beta$ values.
This allows us to ask the question, given a particular merger scenario,
whether the null value (sample mean) is in the confidence
interval.
The Wald statistic that we compute for each model is: 
\begin{align}
	W(\beta) = \frac{\bar{s}_{obs} - \mu(\beta)}{\sigma / (n)^{1/2}} 
\end{align}
where $\bar{x}$ is the sample mean, $\mu(\beta)$, $\sigma$ and $n$ are the
population mean, population standard deviation and size of data of the
simulated model respectively. A higher Wald statistic value would represent
a larger distance between the observed and model value. Within the range of
