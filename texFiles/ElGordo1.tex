...
From the double radio relic observation and the X-ray wake morphology, 
...
than away from each other. This merging scenario might help
explain why the southeast dark matter lensing peak of El Gordo is
closer to the merger center than the southeast galaxy number density peak. 
However, a more concrete
conclusion would require better knowledge of the time evolution of the
relic propagation speed. Finally, we provide a comparison of El Gordo with
other merging clusters such as the Bullet Cluster and the Musketball Cluster. 
\end{abstract}
\begin{keywords}
...
considered major mergers of subclusters of significant masses. The inferred
...
in the observables of El Gordo and the Bullet can be explained.\par
...
lead the cold core by $\sim 0.08~\mega$pc, assuming they are propagating outwards.
On the other hand, indirect observables of the merger shocks of
El Gordo can only be detected through the radio relic, and the shock is
further offset from the corresponding subcluster ($\sim 0.5~\mega$pc) and
the cool core ($\sim 0.4~\mega$pc). (Should discuss the Mach number,
the speed of sounds in each cluster, and the DM velocities) \par 
Second, for the Bullet Cluster, the cool core (or the bullet) is closer to the
interior of the cluster than the corresponding dark matter subcluster mass
peak, whereas, the cool core of El Gordo is further offset from the center of
mass than the corresponding SE subcluster (See Fig.~\ref{fig:config}). 
...
later stage of a merger. \par 
(maybe I should depict the following scenario in a
cartoon and use the passage as a caption) Shortly after the core-passage
...
cool core to lag behind the DM peak. As time went by, the DM and the cool core of
...
apoapsis and started returning for a second core-passage. The cool core
would then be able to travel further away from the center of mass than
the DM component. Simulation of a major merger by \cite{Mathis05} with
similar mass and mass ratio as El Gordo confirms this picture: it shows the
turn-around of the cool core can occur after the apoapsis of the DM
component, resulting in the cool core being further
